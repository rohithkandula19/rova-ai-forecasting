"""
ROVA AI Forecasting — PostgreSQL Models
All models use PostgreSQL-native types: UUID, ARRAY, JSONB
"""
import uuid
from datetime import datetime, date
from sqlalchemy import (
    String, Boolean, Integer, Float, BigInteger,
    DateTime, Date, ForeignKey, Text, Index,
)
...
class Draw(Base):
    ...
    numbers: Mapped[list] = mapped_column(ARRAY(Integer), nullable=False)  # e.g. [14, 23, 31, 42, 58, 66]
...
class NumberFeature(Base):
    """
    Precomputed statistical features per number per game.
    Refreshed by Celery beat task every 24h.
    """
    __tablename__ = "number_features"
    __table_args__ = (
        Index("ix_features_game_number", "game_id", "number"),
    )
...
class MLModel(Base):
    """Tracks trained model versions — synced with MLflow registry."""
...
class Simulation(Base):
    """Monte Carlo simulation results stored as JSONB."""
    __tablename__ = "simulations"
    __table_args__ = (
        Index("ix_simulations_user_date", "user_id", "ran_at"),
    )
    ...
    n_simulations: Mapped[int] = mapped_column(Integer, default=1_000_000)
...
class BacktestResult(Base):
    """Strategy backtest results stored as JSONB."""
